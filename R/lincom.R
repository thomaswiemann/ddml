...
#' \eqn{\lambda = R'\hat\theta}. Inference is based on the
#' influence function
#'
#' \deqn{\phi_i^\lambda = R' \hat J^{-1} m_i
#'     + \sum_c \theta_c R_{\cdot c}^{IF}(W_i) R_{c \cdot}}
#'
#' where the second term is the delta-method correction for
#' estimated weights (when \code{R_ifmat} is supplied).
#'
...
#' @param R_ifmat An optional \eqn{(n \times p)}{n x p} matrix
#'     of influence functions for the weight vector \eqn{R}.
#'     When supplied, a delta-method correction is applied.
#'     When \code{NULL} (default), \eqn{R} is treated as fixed.
...
#' @export
#' @method lincom ddml
lincom.ddml <- function(fit, R, R_ifmat = NULL,
                         ensemble_idx = 1,
                         labels = NULL, ...) {
  R <- as.matrix(R)
  p <- nrow(fit$coefficients)
  ...
  # Unit-level IF for theta: phi_i = J^{-1} psi_i
  sc <- fit$scores[, , ensemble_idx, drop = FALSE]
  dim(sc) <- dim(sc)[1:2]
  J_j <- fit$J[, , ensemble_idx, drop = FALSE]
  dim(J_j) <- dim(J_j)[1:2]
  theta_if <- sc %*% t(csolve(J_j))  # n x p
  ...
  if (!is.null(R_ifmat)) {
    stopifnot(nrow(R_ifmat) == n, ncol(R_ifmat) == p)
    lincom_if <- lincom_if + R_ifmat %*% (att * R)
  }#IF
...
vcov.ddml_lincom <- function(object, type = "HC1", ...) {
  ...
  if (type == "HC3") {
    stop("HC3 standard errors (generalized leverage) are not ",
         "currently supported for linear combinations.",
         call. = FALSE)
  }#IF
...
lincom.ddml_rep <- function(fit, R, R_ifmat = NULL,
                              ensemble_idx = 1,
                              labels = NULL, ...) {
  # Apply lincom.ddml to each rep
  lc_fits <- lapply(fit$fits, function(f) {
    lincom(f, R = R, R_ifmat = R_ifmat, ensemble_idx = ensemble_idx, 
           labels = labels)
  })
