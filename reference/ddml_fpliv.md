...
- learners_DXZ, learners_DX:
  ...
  of \\E\[D \vert X, Z\]\\, \\E\[D \vert X\]\\. Setup is identical to
  `learners`.
...
`ddml_fpliv` returns an object of S3 class `ddml_fpliv` and `ddml`. See
...
`learners`, `learners_DXZ`, `learners_DX`.

## Details

**Parameter of Interest:** `ddml_fpliv` provides a Double/Debiased
Machine Learning estimator for the flexible partially linear
instrumental variable (IV) coefficient \\\theta_0\\, defined by the
partially linear IV model:

\$\$Y = \theta_0 D + g_0(X) + \varepsilon, \quad E\[\varepsilon\|X, Z\]
= 0,\$\$

where \\W \equiv (Y, D, X, Z, \varepsilon)\\ is a random vector such
that \\E\[Var(E\[D\|X, Z\]\|X)\] \neq 0\\, and \\g_0(X)\\ is an unknown
nuisance function.

**Neyman Orthogonal Score:** The Neyman orthogonal score is:

\$\$m(W; \theta, \eta) = \[(Y - \ell(X)) - \theta(D - r(X))\](v(X, Z) -
r(X))\$\$

where the nuisance parameters are \\\eta = (\ell, r, v)\\ taking true
values \\\ell_0(X) = E\[Y\|X\]\\, \\r_0(X) = E\[D\|X\]\\, and \\v_0(X,
Z) = E\[D\|X, Z\]\\.
...
#> D1          -2.56e-01   1.88e-01   -1.36     0.17
#> (Intercept) -8.48e-05   6.92e-03   -0.01     0.99
```
