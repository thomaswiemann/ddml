...
- learners_DXZ, learners_DX:
  ...
  X, Z\]\\, \\E\[D \vert X\]\\. Setup is identical to `learners`.
...
- enforce_LIE:
  ...
  Indicator equal to 1 if the law of iterated expectations is enforced
  in the first stage.
...
- custom_ensemble_weights_DXZ, custom_ensemble_weights_DX:
  ...
  `learners_DXZ`,`learners_DX`. Setup is identical to
  `custom_ensemble_weights`. Note: `custom_ensemble_weights` and
  `custom_ensemble_weights_DXZ`,`custom_ensemble_weights_DX` must have
...
`ddml_fpliv` returns an object of S3 class `ddml_fpliv`. An object of
class `ddml_fpliv` is a list containing the following components:
...
- `iv_fit`:
  ...
  \hat{E}\[Y\vert X\]\\ on \\D - \hat{E}\[D\vert X\]\\ using
  \\\hat{E}\[D\vert X,Z\] - \hat{E}\[D\vert X\]\\ as the instrument.

- `learners`,`learners_DX`,`learners_DXZ`,
  `cluster_variable`,`subsamples`, `cv_subsamples_list`,`ensemble_type`:

  Pass-through of selected user-provided arguments. See above.

## Details

`ddml_fpliv` provides a double/debiased machine learning estimator for
the parameter of interest \\\theta_0\\ in the partially linear IV model
given by

\\Y = \theta_0D + g_0(X) + U,\\

where \\(Y, D, X, Z, U)\\ is a random vector such that \\E\[U\vert X,
Z\] = 0\\ and \\E\[Var(E\[D\vert X, Z\]\vert X)\] \neq 0\\, and \\g_0\\
...
#> (Intercept)  0.000466    0.00691  0.0675    0.946
#> D_r         -0.179025    0.17248 -1.0380    0.299
#> 
```
