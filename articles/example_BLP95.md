...
This article revisits the empirical example of Chernozhukov, Hansen, and
...
We consider the automobile market data from Berry, Levinsohn, Pakes
...
The below snippet constructs the BLP1995 instruments, which are sums of
product characteristics (excluding price and other potentially
...
In addition, we may be interested in extending the list of instruments.
The below constructs the additional instruments considered in CHS2015.
...
We begin by computing the OLS and TSLS estimates using the baseline
...
error in the original code of the CHS2015.
...
Similarly, we compute estimates using the expanded set of controls and
instruments.
...
A more ambitious approach may consider a variety of computational models
for the estimation of the first and second stage. Here, we consider a
combination of linear models, including unpenalized regression (see
[`?ols`](https://www.thomaswiemann.com/ddml/reference/ols.md)), lasso
and ridge regression, random forests (see
...
In addition to allowing for different machine learners, we also consider
different sets of instruments and controls: Either the initial set of
instruments as in BLP1995 or the extended set as in CHS2015. For this
purpose, it is convenient to pre-specify sets of indices corresponding
to the initial and extended sets.
...
number of products with inelastic demand as in CHS2015.
...
BLP1995’s TSLS estimates suggest about 746-896 products with inelastic
demand, nearly half of the number of products implied by simple OLS
estimates.
...
In addition to `ddml`, we can also consider estimators from the `hdm`
package based on rigorous lasso – i.e., with plug-in penalty parameters
and without sample-splitting.
...
very few instruments and controls.
...
stacking learner with the pre-selected set of controls and instruments.
...
Berry S, Levinshon J, Pakes A (1995). “Automobile Prices in Market
...
instruments.” American Economic Review, 105(5), 486-490.
