...
```r
ncol_X <- ncol(X_)
#> Error in ncol(X_): object 'X_' not found
...
#> Error in is.data.frame(x): object 'X_' not found
Z_ <- cbind(sum_other, sum_rival); ncol_Z <- ncol(Z_)
cat("Number of baseline controls: ", ncol_X, "\n",
    "Number of baseline instruments: ", ncol_Z)
#> Error in cat("Number of baseline controls: ", ncol_X, "\n", "Number of baseline instruments: ", : object 'ncol_X' not found
```
...
```r
ols_fit <- lm(y ~ D + X_)
#> Error in eval(predvars, data, env): object 'X_' not found
round(summary(ols_fit)$coefficients[2, ], 4)
#> Error in summary(ols_fit): object 'ols_fit' not found

tsls_fit <- ivreg(y ~ D + X_ | X_ + Z_)
#> Error in eval(predvars, data, env): object 'X_' not found
round(summary(tsls_fit)$coefficients[2, ], 4)
#> Error in summary(tsls_fit): object 'tsls_fit' not found
```
...
#> Error in cbind(X_, XL_): object 'X_' not found
#> Error in ncol(X_c): object 'X_c' not found
...
learners <- list(list(fun = ols, # ols with the baseline set
                 ...
                 list(fun = mdl_xgboost, # boosted trees with the baseline set
                      ...
                      assign_X = set_X,
                      assign_Z = set_Z))
#> Error in eval(expr, envir, enclos): object 'set_X' not found
...
stacking_fit <- ddml_fpliv(y, D = D,
                           ...
                           sample_folds = 10,
                           silent = T)
#> Error in nrow(X): object 'X_c' not found
t(round(summary(stacking_fit), 4)[2, , ])
#> Error in summary(stacking_fit): object 'stacking_fit' not found
```
...
#> Error in lapply(X = X, FUN = FUN, ...): object 'stacking_fit' not found
```
...
#> Error in compute_inelastic_demand(ols_fit$coef[2]): object 'ols_fit' not found
...
#> Error in compute_inelastic_demand(tsls_fit$coef[2]): object 'tsls_fit' not found
...
#> Error in compute_inelastic_demand(stacking_fit$coef): object 'stacking_fit' not found
```
...
#> Error in rlassoIV(x = XL_, d = D, y = y, z = ZL_, select.Z = TRUE, select.X = FALSE, : could not find function "rlassoIV"
summary(rlassoIV_fit)
#> Error in summary(rlassoIV_fit): object 'rlassoIV_fit' not found
```
...
#> Error in which(rlassoIV_fit$selected[1:48]): object 'rlassoIV_fit' not found
dim(Zr_)[2]
#> Error in eval(expr, envir, enclos): object 'Zr_' not found
Xr_ <- XL_[, which(rlassoIV_fit$selected[49:length(rlassoIV_fit$selected)])]
#> Error in which(rlassoIV_fit$selected[49:length(rlassoIV_fit$selected)]): object 'rlassoIV_fit' not found
dim(Xr_)[2]
#> Error in eval(expr, envir, enclos): object 'Xr_' not found
```
...
stacking_r_fit <- ddml_fpliv(y, D = D,
                        ...
                        sample_folds = 10,
                        silent = T)
#> Error in nrow(X): object 'Xr_' not found
round(summary(stacking_r_fit)[2, , 1], 4)
#> Error in summary(stacking_r_fit): object 'stacking_r_fit' not found
```
...
#> Error in lapply(X = X, FUN = FUN, ...): object 'stacking_r_fit' not found
```
